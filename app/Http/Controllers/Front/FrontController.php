...
class FrontController extends Controller
{
    ...
    public function aboutUs()
    {
        ...
        $aboutUs = AboutUs::query()->get()->first();
    ...
    public function listBlog(Request $request, $slug)
    {
        ...
        $data['cate_des'] = $category->intro ?? '';
        $data['categories'] = PostCategory::with([
            'posts' => function ($query) {
                $query->where(['status' => 1])->limit(10)->get();
            }
        ])->where(['parent_id' => 0, 'show_home_page' => 1])->get();
        $data['newBlogs'] = Post::with(['image'])->where(['status' => 1])
            ->orderBy('id', 'DESC')
    ...
    public function policyDetail($slug)
    {
        ...
        return view('site.policies', compact('content', 'title', 'policies', 'description'));
    }
}
